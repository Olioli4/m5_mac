...
ssh $MAC_HOST "export PATH=/usr/local/bin:/usr/local/opt/automake/bin:/usr/local/opt/libtool/bin:`$PATH && cd $REMOTE_DIR && arch -x86_64 $FLUTTER pub get"
...
ssh $MAC_HOST "export PATH=/usr/local/bin:/usr/local/opt/automake/bin:/usr/local/opt/libtool/bin:`$PATH && cd $REMOTE_DIR && arch -x86_64 $FLUTTER build macos --release"
